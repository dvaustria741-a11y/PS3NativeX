...
{
#if defined(__ANDROID__)
	// The larger AUDIO_MIN_LATENCY buffer (see the .h) only buys so much
	// slack: it doesn't help if this callback thread runs at normal
	// priority and gets starved of CPU time by the PPU/SPU/RSX emulation
	// threads on a loaded mobile SoC, still missing its deadline and
	// producing an audible glitch regardless of buffer size. Cubeb creates
	// its own OS thread(s) for this callback with no creation-time hook
	// available to us, so elevate priority lazily on first entry instead -
	// thread_local rather than a single static, since a stream
	// restart (Close/Open, or pause/resume on some backends) can spin up a
	// genuinely different underlying OS thread that needs its own
	// elevation. -19 matches Android's ANDROID_PRIORITY_URGENT_AUDIO.
	static thread_local const bool s_priority_raised = []{
		::setpriority(PRIO_PROCESS, static_cast<id_t>(::gettid()), -19);
		return true;
	}();
	static_cast<void>(s_priority_raised);
#endif
